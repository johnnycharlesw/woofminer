...
if ($tables_views && !$error && !$_POST["search"]) {
	...
	} elseif ($result = queries(($_POST["optimize"] ? "OPTIMIZE" : ($_POST["check"] ? "CHECK" : ($_POST["repair"] ? "REPAIR" : "ANALYZE"))) . " TABLE " . implode(", ", array_map('Woofminer\idf_escape', $_POST["tables"])))) {
	...
	if ($_GET["ns"] !== "") {
		...
		if (!$tables_list) {
			...
			if (support("table")) {
				...
				echo html_select("op", woofminer()->operators(), idx($_POST, "op", JUSH == "elastic" ? "should" : "LIKE %%"));
			...
			if (!information_schema(DB)) {
				...
				$databases = (support("scheme") ? woofminer()->schemas() : woofminer()->databases());
				echo "</div></fieldset>\n";
				$script = "";
