...
if ($_POST && !$error) {
	...
	if ($is_sql) {
		echo "-- Woofminer " . VERSION . " " . get_driver(DRIVER) . " " . str_replace("\n", " ", connection()->server_info) . " dump\n\n";
		if (JUSH == "sql") {
			echo "SET NAMES utf8;
SET time_zone = '+00:00';
			...
			if ($_POST["table_style"] || $_POST["data_style"]) {
				...
				foreach (table_status('', true) as $name => $table_status) {
					...
					if ($table || $data) {
						...
						woofminer()->dumpTable($name, ($table ? $_POST["table_style"] : ""), (is_view($table_status) ? 2 : 0));
						if (is_view($table_status)) {
							$views[] = $name;
						} elseif ($data) {
							$fields = fields($name);
							woofminer()->dumpData($name, $_POST["data_style"], "SELECT *" . convert_fields($fields, $fields) . " FROM " . table($name));
						}
				...
				if (function_exists('Woofminer\foreign_keys_sql')) {
					foreach (table_status('', true) as $name => $table_status) {
						$table = (DB == "" || in_array($name, (array) $_POST["tables"]));
				...
				foreach ($views as $view) {
					woofminer()->dumpTable($view, $_POST["table_style"], 1);
				}
...
echo "<tr><th>" . lang('Output') . "<td>" . html_radios("output", woofminer()->dumpOutput(), $row["output"]) . "\n";

echo "<tr><th>" . lang('Format') . "<td>" . html_radios("format", woofminer()->dumpFormat(), $row["format"]) . "\n";
