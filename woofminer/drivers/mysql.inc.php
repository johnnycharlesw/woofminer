...
if (!defined('Woofminer\DRIVER')) {
	define('Woofminer\DRIVER', "server"); // server - backwards compatibility
	...
	*/
	function fields(string $table): array {
		...
		foreach (get_rows("SELECT * FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = " . q($table) . " ORDER BY ORDINAL_POSITION") as $row) {
			...
			$extra = $row["EXTRA"];
			// https://mariadb.com/kb/en/library/show-columns/, https://github.com/johnnycharlesw/woofminer/pull/359#pullrequestreview-276677186
		...
		if ($create_table) {
			...
			foreach ($matches as $match) {
				...
				$return[idf_unescape($match[1])] = array(
					...
					"source" => array_map('Woofminer\idf_unescape', $source[0]),
					"target" => array_map('Woofminer\idf_unescape', $target[0]),
	...
	*/
	function drop_views(array $views) {
		return queries("DROP VIEW " . implode(", ", array_map('Woofminer\table', $views)));
	}
	...
	*/
	function drop_tables(array $tables) {
		return queries("DROP TABLE " . implode(", ", array_map('Woofminer\table', $tables)));
	}
