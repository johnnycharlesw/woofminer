...
if (isset($_GET["pgsql"])) {
	...
	if (extension_loaded("pgsql") && $_GET["ext"] != "pdo") {
		class PgsqlDb extends SqlDb {
			...
			function select_db(string $database) {
				if ($database == woofminer()->database()) {
		...
		class PgsqlDb extends PdoDb {
			...
			function select_db(string $database) {
				return (woofminer()->database() == $database);
			}
	...
	if (class_exists('Woofminer\PgsqlDb')) {
	...
	class Driver extends SqlDriver {
		...
		static function connect(string $server, string $username, string $password) {
			...
			if (min_version(9, 0, $connection)) {
				$connection->query("SET application_name = 'Woofminer'");
			}
		...
		function partitionsInfo(string $table): array {
			...
			if ($row) {
				...
				return array(
					...
					"partition" => implode(", ", array_map('Woofminer\idf_escape', $attrs)),
				);
			}
			return array();
		}
...
ORDER BY conkey, conname") as $row
			...
			if (preg_match('~FOREIGN KEY\s*\((.+)\)\s*REFERENCES (.+)\((.+)\)(.*)$~iA', $row['definition'], $match)) {
				$row['source'] = array_map('Woofminer\idf_unescape', array_map('trim', explode(',', $match[1])));
				if (preg_match('~^(("([^"]|"")+"|[^"]+)\.)?"?("([^"]|"")+"|[^"]+)$~', $match[2], $match2)) {
					$row['ns'] = idf_unescape($match2[2]);
					$row['table'] = idf_unescape($match2[4]);
				}
				$row['target'] = array_map('Woofminer\idf_unescape', array_map('trim', explode(',', $match[3])));
	...
	function drop_databases($databases) {
		connection()->close();
		return apply_queries("DROP DATABASE", $databases, 'Woofminer\idf_escape');
	}
	...
	function truncate_tables($tables) {
		return queries("TRUNCATE " . implode(", ", array_map('Woofminer\table', $tables)));
	}
	...
	function create_sql($table, $auto_increment, $style) {
		...
		foreach (indexes($table) as $index_name => $index) {
			if ($index['type'] == 'PRIMARY') {
				$primary = $index_name;
				$return_parts[] = "CONSTRAINT " . idf_escape($index_name) . " PRIMARY KEY (" . implode(', ', array_map('Woofminer\idf_escape', $index['columns'])) . ")";
			}
		}
