...
if (isset($_GET["sqlite"])) {
	define('Woofminer\DRIVER', "sqlite");
	...
	if (class_exists('Woofminer\SqliteDb')) {
	...
	function create_database($db, $collation) {
		...
		$link->query('CREATE TABLE woofminer (i)'); // otherwise creates empty file
		$link->query('DROP TABLE woofminer');
		return true;
	}
	...
	*/
	function recreate_table(string $table, string $name, array $fields, array $originals, array $foreign, string $auto_increment = "", $indexes = array(), string $drop_check = "", string $add_check = ""): bool {
		...
		if ($table != "") {
			if ($originals && !queries("INSERT INTO " . table($temp_name) . " (" . implode(", ", $originals) . ") SELECT " . implode(", ", array_map('Woofminer\idf_escape', array_keys($originals))) . " FROM " . table($table))) {
				return false;
			}
			$triggers = array();
	...
	function create_sql($table, $auto_increment, $style) {
		...
		foreach (indexes($table) as $name => $index) {
			...
			$return .= ";\n\n" . index_sql($table, $index['type'], $name, "(" . implode(", ", array_map('Woofminer\idf_escape', $index['columns'])) . ")");
		}
		return $return;
	}
