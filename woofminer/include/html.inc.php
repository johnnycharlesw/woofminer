...
function enum_input(string $type, string $attrs, array $field, $value, string $empty = ""): string {
	...
	foreach ($matches[1] as $val) {
		...
		$return .= " <label><input type='$type'$attrs value='" . h($prefix . $val) . "'" . ($checked ? ' checked' : '') . '>' . h(woofminer()->editVal($val, $field)) . '</label>';
	}
	return $return;
}
...
function input(array $field, $value, ?string $function, ?bool $autofocus = false): void {
	...
	$functions = (isset($_GET["select"]) || $reset ? array("orig" => lang('original')) : array()) + woofminer()->editFunctions($field);
	...
	if ($field["type"] == "enum") {
		echo h($functions[""]) . "<td>" . woofminer()->editInput($table, $field, $attrs, $value);
	} else {
		...
		$input = woofminer()->editInput($table, $field, $attrs, $value); // usage in call is without a table
		if ($input != "") {
			echo $input;
		...
		echo woofminer()->editHint($table, $field, $value);
		// skip 'original'
		$first = 0;
...
function process_input(array $field) {
	...
	return woofminer()->processInput($field, $value, $function);
}
...
function search_tables(): void {
	...
	foreach (table_status('', true) as $table => $table_status) {
		$name = woofminer()->tableName($table_status);
		if (isset($table_status["Engine"]) && $name != "" && (!$_POST["tables"] || in_array($table, $_POST["tables"]))) {
			$result = connection()->query("SELECT" . limit("1 FROM " . table($table), " WHERE " . implode(" AND ", woofminer()->selectSearchProcess(fields($table), array())), 1));
...
function edit_form(string $table, array $fields, $row, ?bool $update, string $error = ''): void {
	$table_name = woofminer()->tableName(table_status1($table, true));
	page_header(
	...
	woofminer()->editRowPrint($table, $fields, $row, $update);
	if ($row === false) {
	...
	if (!$fields) {
		...
		foreach ($fields as $name => $field) {
			echo "<tr><th>" . woofminer()->fieldName($field);
			...
			if (!$_POST["save"] && is_string($value)) {
				$value = woofminer()->editVal($value, $field);
			}
			$function = ($_POST["save"]
		...
		if (!support("table") && !fields($table)) {
			echo "<tr>"
				...
				. "<td class='function'>" . html_select("field_funs[]", woofminer()->editFunctions(array("null" => isset($_GET["select"]))))
