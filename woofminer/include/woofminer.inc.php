...
// any method change in this file should be transferred to editor/include/woofminer.inc.php

/** Default Woofminer plugin; it should call methods via woofminer()->f() instead of $this->f() to give chance to other plugins */
class Woofminer {
	/** @var Woofminer|Plugins */ static $instance;
	...
	*/
	function name(): string {
		return "<a href='https://www.github.com/johnnycharlesw/woofminer/wiki/'" . target_blank() . " id='h1'><img src='../woofminer/static/logo.png' width='24' height='24' alt='' id='logo'>Woofminer</a>";
	}
	...
	*/
	...
	function loginForm(): void {
		...
		// this is matched by compile.php
		echo woofminer()->loginFormField('driver', '<tr><th>' . lang('System') . '<td>', html_select("auth[driver]", SqlDriver::$drivers, DRIVER, "loginDriver(this);"));
		echo woofminer()->loginFormField('server', '<tr><th>' . lang('Server') . '<td>', '<input name="auth[server]" value="' . h(SERVER) . '" title="hostname[:port]" placeholder="localhost" autocapitalize="off">');
		// this is matched by compile.php
		echo woofminer()->loginFormField('username', '<tr><th>' . lang('Username') . '<td>', '<input name="auth[username]" id="username" autofocus value="' . h($_GET["username"]) . '" autocomplete="username" autocapitalize="off">' . script("const authDriver = qs('#username').form['auth[driver]']; authDriver && authDriver.onchange();"));
		...
		echo woofminer()->loginFormField('db', '<tr><th>' . lang('Database') . '<td>', '<input name="auth[db]" value="' . h($_GET["db"]) . '" autocapitalize="off">');
		echo "</table>\n";
	...
	*/
	function login(string $login, string $password) {
		if ($password == "") {
			return lang('Woofminer does not support accessing a database without a password, <a href="https://www.github.com/johnnycharlesw/woofminer/wiki/en/password/"%s>more information</a>.', target_blank());
		}
		return true;
	}
	...
	function selectSearchPrint(array $where, array $columns, array $indexes): void {
		...
		foreach ($indexes as $i => $index) {
			if ($index["type"] == "FULLTEXT") {
				echo "<div>(<i>" . implode("</i>, <i>", array_map('Woofminer\h', $index["columns"])) . "</i>) AGAINST";
		...
		foreach (array_merge((array) $_GET["where"], array(array())) as $i => $val) {
			if (!$val || ("$val[col]$val[val]" != "" && in_array($val["op"], woofminer()->operators()))) {
				...
				echo html_select("where[$i][op]", woofminer()->operators(), $val["op"], $change_next);
	...
	function selectSearchProcess(array $fields, array $indexes): array {
		...
		foreach ($indexes as $i => $index) {
			if ($index["type"] == "FULLTEXT" && idx($_GET["fulltext"], $i) != "") {
				$return[] = "MATCH (" . implode(", ", array_map('Woofminer\idf_escape', $index["columns"])) . ") AGAINST (" . q($_GET["fulltext"][$i]) . (isset($_GET["boolean"][$i]) ? " IN BOOLEAN MODE" : "") . ")";
			}
		}
		foreach ((array) $_GET["where"] as $key => $val) {
			$col = $val["col"];
			if ("$col$val[val]" != "" && in_array($val["op"], woofminer()->operators())) {
				$conds = array();
				foreach (($col != "" ? array($col => $fields[$col]) : $fields) as $name => $field) {
					...
					if (preg_match('~IN$~', $val["op"])) {
						...
						$cond = " $match[1] " . woofminer()->processInput($field, "%$val[val]%");
					} elseif ($val["op"] == "FIND_IN_SET") {
						$prefix = "$val[op](" . q($val["val"]) . ", ";
						$cond = ")";
					} elseif (!preg_match('~NULL$~', $val["op"])) {
						$cond .= " " . woofminer()->processInput($field, $val["val"]);
					}
			...
			if ($result) {
				...
				while ($row = $result->$fetch_function()) {
					...
					if ($_POST["format"] != "sql") {
						...
						if (!$insert) {
							$insert = "INSERT INTO " . table($table) . " (" . implode(", ", array_map('Woofminer\idf_escape', $keys)) . ") VALUES";
						}
	...
	*/
	...
	/** Print navigation after Woofminer title
	...
	*/
	function navigation(string $missing): void {
		...
		echo " <a href='https://www.github.com/johnnycharlesw/woofminer/wiki/#download'" . target_blank() . " id='version'>" . (version_compare(VERSION, $new_version) < 0 ? h($new_version) : "") . "</a>";
		echo "</span></h1>\n";
		// this is matched by compile.php
		switch_lang();
		if ($missing == "auth") {
					...
					foreach ($usernames as $username => $password) {
						if ($password !== null) {
							...
							foreach (($dbs ? array_keys($dbs) : array("")) as $db) {
								$output .= "<li><a href='" . h(auth_url($vendor, $server, $username, $db)) . "'>($name) " . h("$username@" . ($server != "" ? woofminer()->serverName($server) : "") . ($db != "" ? " - $db" : "")) . "</a>\n";
							}
						}
					}
				}
			...
			woofminer()->syntaxHighlighting($tables);
	...
	function databasesPrint(string $missing): void {
		...
		if (support("scheme")) {
			if ($missing != "db" && DB != "" && connection()->select_db(DB)) {
				echo "<br><label>" . lang('Schema') . ": " . html_select("ns", array("" => "") + woofminer()->schemas(), $_GET["ns"]) . "$db_events</label>";
				if ($_GET["ns"] != "") {
					set_schema($_GET["ns"]);
				}
			}
	...
	*/
	function tablesPrint(array $tables): void {
		...
		foreach ($tables as $table => $status) {
			...
			$name = woofminer()->tableName($status);
