...
class Plugins {
	...
	/** Register plugins
	* @param ?list<object> $plugins object instances or null to autoload plugins from woofminer-plugins/
	*/
	function __construct(?array $plugins) {
		if ($plugins === null) {
			$plugins = array();
			$basename = "woofminer-plugins";
			if (is_dir($basename)) {
				foreach (glob("$basename/*.php") as $filename) {
					$include = include_once "./$filename";
				}
			}
			$help = " href='https://www.github.com/johnnycharlesw/woofminer/wiki/plugins/#use'" . target_blank();
			...
			foreach (get_declared_classes() as $class) {
				if (!$plugins[$class] && preg_match('~^Woofminer\w~i', $class)) {
		...
		$woofminer = new Woofminer;
		$plugins[] = $woofminer;
		$reflection = new \ReflectionObject($woofminer);
