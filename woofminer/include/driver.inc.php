...
abstract class SqlDriver {
	...
	* @param list<string> $select result of woofminer()->selectColumnsProcess()[0]
	* @param list<string> $where result of woofminer()->selectSearchProcess()
	* @param list<string> $group result of woofminer()->selectColumnsProcess()[1]
	* @param list<string> $order result of woofminer()->selectOrderProcess()
	* @param int $limit result of woofminer()->selectLimitProcess()
	...
	*/
	function select(string $table, array $select, array $where, array $group, array $order = array(), int $limit = 1, ?int $page = 0, bool $print = false) {
		...
		$query = woofminer()->selectQueryBuild($select, $where, $group, $order, $limit, $page);
		if (!$query) {
			$query = "SELECT" . limit(
		...
		if ($print) {
			echo woofminer()->selectQuery($query, $start, !$return);
		}
		return $return;
	}
