...
function add_invalid_login(): void {
	$base = get_temp_dir() . "/woofminer.invalid";
	// woofminer.invalid may not be writable by us, try the files with random suffixes
	...
	$invalid = &$invalids[woofminer()->bruteForceKey()];
	if (!$invalid) {
...
function check_invalid_login(array &$permanent): void {
	$invalids = array();
	foreach (glob(get_temp_dir() . "/woofminer.invalid*") as $filename) {
		$fp = file_open_lock($filename);
		if ($fp) {
			$invalids = unserialize(stream_get_contents($fp));
			file_unlock($fp);
	...
	/** @var array{int, int} */
	$invalid = idx($invalids, woofminer()->bruteForceKey(), array());
...
if ($auth) {
	...
	if ($auth["permanent"]) {
		...
		$private = woofminer()->permanentLogin(true);
	...
	redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~', '', ME), 0, -1), lang('Logout successful.') . ' ' . lang('Thanks for using Woofminer, consider <a href="https://www.github.com/johnnycharlesw/woofminer/wiki/en/donation/">donating</a>.'));

} elseif ($permanent && !$_SESSION["pwds"]) {
	session_regenerate_id();
	$private = woofminer()->permanentLogin();
...
function auth_error(string $error, array &$permanent) {
	...
	if (isset($_GET["username"])) {
		...
		if (($_COOKIE[$session_name] || $_GET[$session_name]) && !$_SESSION["token"]) {
			...
			if ($password !== null) {
				if ($password === false) {
					$error .= ($error ? '<br>' : '') . lang('Master password expired. <a href="https://www.github.com/johnnycharlesw/woofminer/wiki/en/extension/"%s>Implement</a> %s method to make it permanent.', target_blank(), '<code>permanentLogin()</code>');
				}
...
if (isset($_GET["username"]) && !class_exists('Woofminer\Db')) {
...
$login = null;
if (!is_object($connection) || ($login = woofminer()->login($_GET["username"], get_password())) !== true) {
...
if ($_POST) {
	if (!verify_token()) {
		...
		$error = (!$_POST["token"] && $max_vars
			...
			: lang('Invalid CSRF token. Send the form again.') . ' ' . lang('If you did not send this request from Woofminer then close this page.')
		);
	}

} elseif ($_SERVER["REQUEST_METHOD"] == "POST") {
	// posted form with no data means that post_max_size exceeded because Woofminer always sends token at least
