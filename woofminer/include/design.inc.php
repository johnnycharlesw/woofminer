...
function page_header(string $title, string $error = "", $breadcrumb = array(), string $title2 = ""): void {
	...
	$title_page = strip_tags($title_all . (SERVER != "" && SERVER != "localhost" ? h(" - " . SERVER) : "") . " - " . woofminer()->name());
...
<link rel="stylesheet" href="../woofminer/static/default.css">
<?php
	...
	$dark = ($has_light
		? ($has_dark ? null : false) // both styles - autoswitching, only woofminer.css - light
		: ($has_dark ?: null) // only woofminer-dark.css - dark, neither - autoswitching
	);
	$media = " media='(prefers-color-scheme: dark)'";
	if ($dark !== false) {
		echo "<link rel='stylesheet'" . ($dark ? "" : $media) . " href='../woofminer/static/dark.css'>\n";
	}
	echo "<meta name='color-scheme' content='" . ($dark === null ? "light dark" : ($dark ? "dark" : "light")) . "'>\n";

	// this is matched by compile.php
	echo script_src("../woofminer/static/functions.js");
	echo script_src("static/editing.js");
	if (woofminer()->head($dark)) {
		...
		echo "<link rel='apple-touch-icon' href='../woofminer/static/logo.png'>\n";
	}
...
const thousandsSeparator = '" . js_escape(lang(',')) . "';")
	...
	if ($breadcrumb !== null) {
		...
		$server = woofminer()->serverName(SERVER);
	...
	stop_session();
	define('Woofminer\PAGE_HEADER', 1);
}
...
function page_headers(): void {
	...
	foreach (woofminer()->csp(csp()) as $csp) {
		$header = array();
...
function csp(): array {
	return array(
		array(
			...
			"connect-src" => "'self'",
			"frame-src" => "https://www.github.com/johnnycharlesw/woofminer/wiki",
			"object-src" => "'none'",
			"base-uri" => "'none'",
			"form-action" => "'self'",
		),
...
function page_messages(string $error): void {
	...
	if (woofminer()->error) { // separate <div>
		echo "<div class='error'>" . woofminer()->error . "</div>\n";
	}
}
