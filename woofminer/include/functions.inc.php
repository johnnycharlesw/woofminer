...
// This file is used both in Woofminer and Woofminer Editor.
...
/** Get Woofminer object
* @return Woofminer|Plugins
*/
function woofminer() {
	return Woofminer::$instance;
}
...
function get_rows(string $query, ?Db $connection2 = null, string $error = "<p class='error'>"): array {
	...
	} elseif (!$result && !$connection2 && $error && (defined('Woofminer\PAGE_HEADER') || $error == "-- ")) {
		echo $error . error() . "\n";
	}
	return $return;
}
...
function query_redirect(string $query, ?string $location, string $message, $redirect = true, bool $execute = true, bool $failed = false, string $time = ""): bool {
	...
	$sql = ($query ? woofminer()->messageQuery($query, $time, $failed) : "");
	if ($failed) {
		woofminer()->error .= error() . $sql . script("messagesPrint();") . "<br>";
		return false;
	}
	if ($redirect) {
...
*/
function apply_queries(string $query, array $tables, $escape = 'Woofminer\table'): bool {
...
function column_foreign_keys(string $table): array {
	$return = array();
	foreach (woofminer()->foreignKeys($table) as $foreign_key) {
...
function dump_headers(string $identifier, bool $multi_table = false): string {
	$return = woofminer()->dumpHeaders($identifier, $multi_table);
	$output = $_POST["output"];
	if ($output != "text") {
		header("Content-Disposition: attachment; filename=" . woofminer()->dumpFilename($identifier) . ".$return" . ($output != "file" && preg_match('~^[0-9a-z]+$~', $output) ? ".$output" : ""));
	}
	session_write_close();
	if (!ob_get_level()) {
		ob_start(null, 4096);
...
/** Read password from file woofminer.key in temporary directory or create one
* @return string '' if the file can not be created
*/
function password_file(bool $create): string {
	$filename = get_temp_dir() . "/woofminer.key";
...
function select_value($val, string $link, array $field, ?string $text_length): string {
	...
	if (!$link) {
		$link = woofminer()->selectLink($val, $field);
	}
	if ($link === null) {
		if (is_mail($val)) {
			$link = "mailto:$val";
	...
	$return = woofminer()->editVal($val, $field);
	if ($return !== null) {
		if (!is_utf8($return)) {
	...
	return woofminer()->selectVal($return, $link, $field, $val);
}
...
function slow_query(string $query): array {
	...
	$timeout = woofminer()->queryTimeout();
