...
woofminer()->selectLinks($table_status, (isset($rights["insert"]) || !support("table") ? "" : null));
...
if ($fields) {
	woofminer()->tableStructurePrint($fields, $table_status);
}
...
if (support("indexes") && driver()->supportsIndex($table_status)) {
	...
	if ($indexes) {
		woofminer()->tableIndexesPrint($indexes, $table_status);
	}
	...
	if (fk_support($table_status)) {
		...
		if ($foreign_keys) {
			...
			foreach ($foreign_keys as $name => $foreign_key) {
				...
				echo "<th><i>" . implode("</i>, <i>", array_map('Woofminer\h', $foreign_key["source"])) . "</i>";
				$link = ($foreign_key["db"] != ""
				...
				echo "<td><a href='" . h($link . "table=" . urlencode($foreign_key["table"])) . "'>"
					...
					. "</a>"
				;
				echo "(<i>" . implode("</i>, <i>", array_map('Woofminer\h', $foreign_key["target"])) . "</i>)";
