...
list($select, $group) = woofminer()->selectColumnsProcess($columns, $indexes);
$select = array_unique($select);
$group = array_unique($group);
$is_group = count($group) < count($select);
$where = woofminer()->selectSearchProcess($fields, $indexes);
$order = woofminer()->selectOrderProcess($fields, $indexes);
$limit = woofminer()->selectLimitProcess();
...
if ($_POST && !$error) {
	...
	if ($_POST["export"]) {
		...
		woofminer()->dumpData($TABLE, "table", $query);
		woofminer()->dumpFooter();
		exit;
	}

	if (!woofminer()->selectEmailProcess($where, $foreign_keys)) {
			...
			if (!$_POST["val"]) {
				...
				foreach ($_POST["val"] as $unique_idf => $row) {
					...
					foreach ($row as $key => $val) {
						...
						$set[idf_escape($key)] = (preg_match('~char|text~', $fields[$key]["type"]) || $val != "" ? woofminer()->processInput($fields[$key], $val) : "NULL");
					}
...
$table_name = woofminer()->tableName($table_status);
if (is_ajax()) {
	page_headers();
	ob_start();
} else {
...
woofminer()->selectLinks($table_status, $set);
...
if (!$columns && support("table")) {
	...
	echo "</div>\n";
	woofminer()->selectColumnsPrint($select, $columns);
	woofminer()->selectSearchPrint($where, $search_columns, $indexes);
	woofminer()->selectOrderPrint($order, $order_columns, $indexes);
	woofminer()->selectLimitPrint($limit);
	woofminer()->selectLengthPrint($text_length);
	woofminer()->selectActionPrint($indexes);
	echo "</form>\n";
	...
	if (!$result) {
		...
		if (!$rows) {
			...
			$backward_keys = woofminer()->backwardKeys($TABLE, $table_name);
			...
			foreach ($rows[0] as $key => $val) {
				if (!isset($unselected[$key])) {
					...
					$name = ($field ? woofminer()->fieldName($field, $rank) : ($val["fun"] ? "*" : h($key)));
					if ($name != "") {
						$rank++;
						$names[$key] = $name;
			...
			foreach (woofminer()->rowDescriptions($rows, $foreign_keys) as $n => $row) {
				...
				woofminer()->backwardKeysPrint($backward_keys, $rows[$n]);
		...
		if (!is_ajax()) {
			if ($rows || $page) {
				...
				if (woofminer()->selectCommandPrint()) {
					?>
...
<?php
				...
				woofminer()->selectEmailPrint(array_filter($email_fields, 'strlen'), $columns);
				echo "</div></div>\n";
			}

			if (woofminer()->selectImportPrint()) {
				echo "<p>";
