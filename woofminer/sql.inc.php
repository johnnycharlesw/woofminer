...
if (!$error && $_POST["export"]) {
	...
	if ($_POST["format"] == "sql") {
		...
		woofminer()->dumpTable("", "");
		woofminer()->dumpData("", "table", $_POST["query"]);
...
if (!$error && $_POST) {
	...
	if (!isset($_GET["import"])) {
		...
		$sql_file_path = woofminer()->importServerPath();
			...
			if (!$offset && preg_match("~^$space*+DELIMITER\\s+(\\S+)~i", $query, $match)) {
				...
				if (!$found && $fp && !feof($fp)) {
					...
					if ($found && !preg_match("(^$delimiter)", $found)) { // find matching quote or comment end
						...
						$commands++;
						$print = "<pre id='sql-$commands'><code class='jush-" . JUSH . "'>" . woofminer()->sqlCommandQuery($q) . "</code></pre>\n";
									...
									if (is_object($result)) {
										...
										if (!$_POST["only_errors"]) {
											...
											echo ", <a href='#$id'>" . lang('Export') . "</a>" . script("qsl('a').onclick = partial(toggle, '$id');", "") . "<span id='$id' class='hidden'>: "
												. html_select("output", woofminer()->dumpOutput(), $adminer_export["output"]) . " "
												. html_select("format", woofminer()->dumpFormat(), $adminer_export["format"])
...
if (!isset($_GET["import"])) {
	...
	echo "</div></fieldset>\n";
	$importServerPath = woofminer()->importServerPath();
	if ($importServerPath) {
