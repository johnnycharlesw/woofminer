...
Woofminer loads a database driver based on a URL parameter (e.g., `pgsql=`).
The drivers live in [woofminer/drivers/](/woofminer/drivers/) and [plugins/drivers/](/plugins/drivers/).
The driver consists of the class [`Driver`](/woofminer/include/driver.inc.php) and a set of functions that ideally belong in `Driver` but remain separate due to historical reasons.

A driver also creates the [`Db`](https://github.com/johnnycharlesw/woofminer/blob/v5.0.6/woofminer/drivers/mysql.inc.php#L62) class based on available PHP extensions.
There is no `DriverMysql` or `DbMysqlPdo`; there is always up to one `Driver` and one `Db`.

If the URL contains `username=`, Woofminer attempts to authenticate that user.
...
Woofminer sets cookies as SameSite which adds a second protection but not for vulnerabilities on the same site.
If a POST action succeeds, Woofminer redirects the browser to a GET request to prevent accidental re-submission.
...
For example, if the URL contains `indexes=`, then [woofminer/indexes.inc.php](/woofminer/indexes.inc.php) is loaded.
...
This prevents modifying `$_SESSION` later in the code but allows multiple Woofminer pages to be opened simultaneously, even if one has a long-running query.
...
Woofminer often checks for empty strings using `$table != ""` instead of `!$table`, since table names can be `0`, and `!$table` would fail in such cases.

## Classes, Functions, Variables, Constants

There are 4 main classes: `Driver`, `Db`, `Woofminer` and `Plugins`.
They are described in other sections.

Woofminer defines many functions which are namespaced to prevent collisions.
...
Plugins can access some of them using helper functions like `Woofminer\driver()`.

Woofminer also defines constants in its namespace.
...
Woofminer is highly conservative regarding PHP version requirements.
...
Compatibility is periodically [checked](https://github.com/johnnycharlesw/woofminer/blob/v5.0.6/phpcs.xml#L121).
...
Woofminer aims for backward compatibility, particularly for plugins.
Only significant improvements, such as adding namespaces, justify breaking changes.

## Extending Functionality

Besides driver classes, Woofminer provides the [`Woofminer`](/woofminer/include/woofminer.inc.php) class for customization.
This class enables Woofminer and Woofminer Editor (which lacks DDL support) to share functionality.
Developers can extend this class to implement customizations, as I do for my projects.

A more common method for extending Woofminer is the [`Plugins`](/woofminer/include/plugins.inc.php) class.
A plugin is simply a class defining any methods from [`Woofminer`](/woofminer/include/woofminer.inc.php).
The `Plugins::__call` method calls all registered plugins until one of them returns non-null.

## Code Style

Woofminer follows a strict [coding style](/phpcs.xml), though some choices may seem unusual.
...
Well-used ternary operators enhance readability, but they are sometimes overused in Woofminer.
...
Woofminer has an excessive line length limit of 250 characters.
...
For example, `Db` methods are documented only in [`db.inc.php`](/woofminer/include/db.inc.php), not in the drivers.
...
Woofminer strictly initializes all variables before use, which is [verified](/phpstan.neon).
However, Woofminer relies on the default value of uninitialized array items.
...
Woofminer [silences](/woofminer/include/errors.inc.php) these errors.
...
Although it would be possible to use such a function in Woofminer, the code would still be less readable than the current approach.
...
Woofminer uses `@` only where an error is unavoidable, such as when writing to files.
Even if you check whether a file is writable, a race condition exists between the check and the actual write operation.

## Escaping

Woofminer does not implement automatic escaping.
...
Woofminer prints data immediately to display partial results when a query is slow.

When constructing SQL queries, use `q()` for strings and `idf_escape()` for identifiers.
Woofminer requires full control when constructing queries, making the use of additional helpers challenging.

## Minimalism

Woofminer is minimalist in every aspect - if something is unnecessary, it should not be included.
This philosophy extends to the UI, which remains as uncluttered as possible.
For example, index names are usually irrelevant compared to the columns they reference, so Woofminer displays index names only in `title=""`.
...
Many closing HTML tags are optional (e.g., `</li>` or `</html>`) and Woofminer obviously doesn't print them.

If a feature can be implemented as a plugin, it is only added to the core if it benefits almost everyone.
For example, [sticky table headers](https://github.com/johnnycharlesw/woofminer/issues/918) are useful to all users and have been included, whereas a [dark mode switcher](https://github.com/johnnycharlesw/woofminer/issues/926) would clutter the UI and is only useful for some, so it remains a plugin.

## Dependencies

Woofminer uses [Git submodules](https://git-scm.com/docs/git-submodule) for dependencies, predating [Composer](https://getcomposer.org/) and other package managers.
Submodules simplify development - for example, I can add a feature to the syntax highlighter, commit the change, and immediately use it in Woofminer.
Woofminer commits simply reference the current HEAD of the submodule, avoiding the need for frequent version releases, lock file updates, or other package management tasks.

## Tests

Woofminer does not include unit tests but has extensive [end-to-end tests](/tests/).
...
Woofminer functions without JavaScript but is more user-friendly when JavaScript is enabled.
...
Now, Woofminer registers event handlers using a short `<script>` element immediately following the relevant tag, typically using `qsl()` (query selector last).
...
JavaScript code is split into [functions.js](/woofminer/static/functions.js) (common utilities) and [editing.js](/woofminer/static/editing.js) (specific to Woofminer or Woofminer Editor).
...
Woofminer generates simple HTML and styles it with basic CSS, respecting user preferences for dark mode.
Users can customize styles via `woofminer.css`.
...
Plurals are stored as arrays, with selection logic handled in [lang.inc.php](/woofminer/include/lang.inc.php).

Plugins extending [`Woofminer\Plugin`](/woofminer/include/plugin.inc.php) can use `$this->lang()` and store translations in `$translations = array('en' => array('' => 'Plugin description'))`.

The website translations are managed at https://www.github.com/johnnycharlesw/woofminer/wiki/en/translations/.

## Compilation

Woofminer’s source code is divided into a manageable number of reasonably small files.
...
Includes in Woofminer start with `./` to bypass `include_path`, which is unrelated to compilation.
...
This prevents plugins from overwriting Woofminer’s variables.
...
`compile.php` outputs the compiled file to the current directory, but it does not need to be run from Woofminer’s directory.
I often run it from a separate directory to prepare releases (29 files) or test versions of Woofminer.

## Version Check

Woofminer checks for new versions via [github.com/johnnycharlesw/woofminer/wiki/version/](https://www.github.com/johnnycharlesw/woofminer/wiki/version/), using a signed response to prevent tampering with the version file on the server where an instance of Woofminer runs.
However, this means that github.com/johnnycharlesw/woofminer/wiki has access to the IP addresses of Woofminer installations.
...
There's also a [plugin](/plugins/version-github.php) checking for new versions [from GitHub](https://github.com/johnnycharlesw/woofminer/releases).
...
If a change modifies Woofminer’s behavior for end users, it should be documented in [CHANGELOG](/CHANGELOG.md) in the same commit.
